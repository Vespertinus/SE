...
explicit Vector3::Vector3( const Real af_coordinate[3] ) :
  x( af_coordinate[0] ),
  y( af_coordinate[1] ),
  z( af_coordinate[2] ) { ;; }
...
explicit Vector3::Vector3( const int af_coordinate[3] ) :  
  x = (float)af_coordinate[0],
  y = (float)af_coordinate[1],
  z = (float)af_coordinate[2] { ;; }



explicit Vector3::Vector3( const float * const r ) : x( r[0] ), y( r[1] ), z( r[2] ) { ;; }



explicit Vector3::Vector3( const float scaler ) : x( scaler ), y( scaler ), z( scaler ) { ;; }
...
float * Vector3::Ptr() { return &x; }



/// Pointer accessor for direct copying
const float * Vector3::Ptr() const { return &x; }
...
friend Vector3 Vector3::operator * ( const float fScalar, const Vector3& rkVector ) {
...
friend Vector3::Vector3 operator + (const Vector3& lhs, const float rhs) {
...
friend Vector3 Vector3::operator + (const float lhs, const Vector3& rhs) {
...
/** Returns the square of the length(magnitude) of the vector.
  ...
  the square root.
 */
float Vector3::squaredLength () const { return x * x + y * y + z * z; }
...
/** Returns the distance to another vector.
  ...
  instead.
 */
float Vector3::distance(const Vector3& rhs) const { return (*this - rhs).length(); }
 ...
 */
float Vector3::squaredDistance(const Vector3& rhs) const { return (*this - rhs).squaredLength(); }
 ...
 */
float Vector3::dotProduct(const Vector3& vec) const { return x * vec.x + y * vec.y + z * vec.z; }
 ...
 */
float Vector3::absDotProduct(const Vector3& vec) const {
...
/** Generates a new random vector which deviates from this vector by a
  ...
  afterwards.
 */
Vector3 Vector3::randomDeviant(const Radian& angle, const Vector3& up = Vector3::ZERO ) const {
